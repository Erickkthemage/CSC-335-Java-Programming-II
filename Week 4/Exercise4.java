...
class RegionalBank extends Bank {
  ...
  public RegionalBank(String n, int c, double[][] ra) {
    setName(n);
    setCustomers(c);
    regionalAccounts = ra;
  }
  ...
  public double regionalTotal() {
    double total = 0;
    for (double[] regionalAccountRow : regionalAccounts) {
      for (double regionalAccount : regionalAccountRow) {
        total += regionalAccount;
      }
    }
    return total;
  }
